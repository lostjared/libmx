...
void dump_font(struct SDL_Font *) {
/*	int i,z,p;
	for(p = 0; p <= 127; p++) {
		for(i = 0; i < fnt->mx; i++)
			for(z = 0; z < fnt->my; z++)
...
void SDL_PrintTextScaled(struct SDL_Surface *surf, struct SDL_Font *fnt, int x, int y, int w, int h, Uint32 color, const char *src) {
	...
	for(p = 0; p < (int)strlen(src);  p++) {
		...
		for(i = 0; i < w; i++) {
			for(z = 0; z < h; z++) {
				if(fnt->letters[(int)src[p]].fnt_ptr[SDL_GetFX(fnt,i,w)][SDL_GetFZ(fnt,z,h)] != fnt->tcolor) {
					setpixel(pbuf, (Uint32)offset_x+i, (Uint32)offset_y+z, color, surf->format->BitsPerPixel, surf->pitch);
					width=i;
					if(height < z)
						height=z;
...
int SDL_PrintTextWidth(int *depth, struct SDL_Surface *surf, struct SDL_Font *fnt, int x, int y, int size_width, Uint32 color, const char *src) {
	int prev_x = x;
	int offset_x = prev_x, offset_y = y;
	int width = 0, height = 0;
	int i,z,p;
	...
	for(p = 0; p < (int)strlen(src);  p++) {
		...
		for(i = 0; i < fnt->mx; i++) {
			for(z = 0; z < fnt->my; z++) {
				if(fnt->letters[(int)src[p]].fnt_ptr[i][z] != fnt->tcolor) {
					if(offset_y+z > 0 && offset_x+i > 0 && offset_y+z < surf->h && offset_x+i < size_width)
					setpixel(pbuf, (Uint32)offset_x+i, (Uint32)offset_y+z, color, surf->format->BitsPerPixel, surf->pitch);
					width=i;
					if(height < z)
						height=z;
		...
		if(offset_x > size_width) return 1;
...
int SDL_PrintTextIdle(struct SDL_Surface *surf, struct SDL_Font *fnt, int  x, int y, int size_width,Uint32 color, const char *src)  {
	...
	int prev_x = x;
		...
		for(p = 0; p < (int)strlen(src);  p++) {
			...
			if(offset_x+width >= size_width-10) {
				offset_x = prev_x;
				offset_y += height;
				return 1;
			}
...
/****************************************** printtext for seperation *************************************************************************/


int SDL_PrintTextDepth(int *depth, struct SDL_Surface *surf, struct SDL_Font *fnt, int x, int y, Uint32 color, const char *src) {
	int prev_x = x;
	int offset_x = prev_x, offset_y = y;
	int width = 0, height = 0;
	int i,z,p;
