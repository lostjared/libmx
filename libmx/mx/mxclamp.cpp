...
namespace mx
{
	...
	bool mxClampOutFile::openFile(string fname)
	{
	...
	/* writes files to be compressed to disk
		...
		provides the basic guarantee ( i think )
	...
	*/
	...
	bool mxClampOutFile::writeToFile()
	{
		...
		for(vi = files.begin(), si = file_names.begin(); vi != files.end(); vi++, si++)
		{
			...
			try
			{
				...
				cout << *si << " " <<  vi->size_bytes << "/" << size_compressed << "\n";
			...
			catch (...)
			{
				...
				throw; // propagate exception, must reopen the file before you can rewrite
			}
	...
	void mxClampResourceFile::buildInfo()
	{
		...
		while(!the_file.eof())
		{
			...
			std::cout << "file in archive:  " << i.filename <<  " " << i.size_compressed << " / " << i.size_bytes << " \n";
;
