...
namespace mx3d {
	...
	public:
		union {
			...
			struct { // i know a anonymous struct bad design flaw
	...
	public:
		...
		bool LoadMX(std::string source, vec4D &scale, vec4D &obj_pos, vec4D &rotation) {
			...
			int num_vertex = 0, num_polys = 0, got_vertex = 0;
		...
		// load andre's format
		bool LoadPLG(std::string plg,vec4D &scale, vec4D &obj_pos, vec4D &rotation, unsigned int triz_color = 0xFF) {
			std::fstream file;
			...
			while(!file.eof()) {
				...
				while(counter < num_p) {
					...
					Triangle tri;
					int num = 0;
					tri.color = triz_color;
					sscanf(cur_line.c_str(),"%x %d %d %d %d",(unsigned int*)&tri.state, &num, &tri.vert[0], &tri.vert[1], &tri.vert[2]);
					this->vlist.push_back(tri);
					counter++;
				}
			}
	...
	public:
		...
		void Begin(graphicsInterface &mx) {
			this->mx = &mx;
		...
		void DrawSolidPolys(RenderList &l) {
			...
			for(unsigned int i = 0; i < l.polys.size(); i++) {
				if(l.polys[i].state == 0 || l.polys[i].state == MX_BACKFACE )
					continue;
					this->mx->drawtri(int(l.polys[i].tlist[0].x), int(l.polys[i].tlist[0].y), int(l.polys[i].tlist[1].x), int(l.polys[i].tlist[1].y), int(l.polys[i].tlist[2].x), int(l.polys[i].tlist[2].y),int(l.polys[i].color));
			}
		}
		...
		virtual void DrawClipedLine(int x1, int y1, int x2, int y2, unsigned int color) {
			if(ClipLine(x1,y1,x2,y2))
			{
				//mx->paint.drawline(x1,y1,x2,y2,color);
				mx->drawline(x1,y1,x2,y2,color);
			}
		}
